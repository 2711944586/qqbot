...
- 同一个群友在同一个群同一天抽到同一名选手，不同群独立，第二天刷新。
- 模糊触发也支持：`今天抽个CS选手`、`抽个职业哥`、`每日哥们`、`今天抽谁`。普通 `/player NiKo` 或 “NiKo 现在在哪队” 不会被抽签功能抢走。
- 选手池包含现役与传奇选手，图片 URL 已提前写入资料池，来源是公开 Liquipedia Commons / Wikimedia Commons。
- 输出包含 @、昵称、队伍语境、定位、签位分、直播味短评和选手图。
- 队伍字段写的是“队伍语境”，不是永久阵容。用户问“最新在哪队/最近状态”时应走 `/player 最新 <名字>` 或直接 @ 提问触发联网。
- 输出走本地逻辑，不调用 AI，不临时联网搜图，不影响 @ 必回队列。
- 设计参考了常见“每日老婆/每日抽取”类 bot：当天固定、返回头像和昵称；本项目改成 CS 选手池，适配群聊和玩机器语态。
...
2. 图片 URL 是代码里预填的公开 Liquipedia/Wikimedia 链接，运行时不会联网搜索。
3. 如果 QQ 不显示图片，先看 PM2 日志里 `send_group_msg` 是否报错；多数是 NapCat 图片发送或外链访问问题，不是 AI 问题。
4. 临时解决：重新发一次 `/csplayer`；长期解决：更新代码里的选手图片 URL，跑 `npm run smoke` 确认选手池每个条目都有图片字段。
